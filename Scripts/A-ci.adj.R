...
setwd("/Users/maquellegarcia/Documents/GitHub/DAT_SERC_anaysis/Results")

# Give a name to the files (table and pdf) that will receive the results
arquivo <-"A_ci_fit_SERC"
...
# Rename the address below for the directory where you files are
# Because some Operational Systems (or GoogleDrive) add a "Desktop.ini" file, the function needs those additional arguments(eg. pattern)

  #curvas1 <- grep(dir(path="/Volumes/GoogleDrive/Meu\ Drive/Work/Tese/database/ACi.metodos/dados",   # 
                    #full.names=TRUE,recursive=T), pattern='desktop.ini', inv=T, value=T)
#names(Curve)
  
# Creating a matrix that stores the column names of the text file that will receive the results of this script
  output.names     <- matrix (c("Tree_id","cut_or_uncut","person","Mean.Tleaf","Max.Tleaf",
...
#-----------------------------------------------
setwd("/Users/maquellegarcia/Documents/GitHub/DAT_SERC_analysis/Data")
curvas<-read_excel("data.SERC.xlsx") 
...
names(curvas)
  ...
  sp<-as.data.frame(unique(curvas1[,"un.id"]))
  colnames(sp)<-"sp"
  sp
  ...
  Curve<- subset(curvas1, un.id==sp[i,1])
...
################### making the figures
  ...
  title(cex.main = 1.5,main = c (as.character(unique(Curve[,"un.id"])),paste(df2$Algorithim[um[1]], df2$StartValue[um[1]], df2$Fit[um[1]])))
  ...
  output <- matrix (c (unique(Curve[,"un.id"]),unique(Curve[,"cut.or.uncut"]),unique(Curve[,"Scientist"]),mean(Curve$Tleaf),max(Curve$Tleaf),
  ...
  write.table (output, paste(arquivo, ".txt", sep=""), append=TRUE, sep="\t", row.names=FALSE, col.names=FALSE)
  start_time <- Sys.time() 
  end_time <- Sys.time()  
  ...
  print(paste(round(which(curvas1 %in% i)*100/length(curvas1),1),"% - ",basename(sp[i,1])," - ",round(Tempo,2)))
  }
