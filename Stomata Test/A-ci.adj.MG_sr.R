...
arquivo <-"A_ci_fit_DAT_noback_sr"
...
setwd(paste0(wd, "/Stomata Test"))
dir()
curvas <- read.csv("Aci_no_out_sr.csv", sep = ",", header = TRUE, fileEncoding="latin1")
...
################### making the figures
  ...
  if (identical(type, subset(curvas1, unique_id==sp[i,1])) == TRUE) {
    ...
    results_no_correct <<- results.csv3 %>% 
      mutate(back_filt = "original") #Change this to "original"
...
write.csv(complete_curve, file = paste0(wd, "/Stomata Test/curve_fits_sr.csv"), #Had trouble with the wd situation
          row.names = FALSE)
